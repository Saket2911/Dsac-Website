...
async function fetchCodeforcesContests() {
  try {
    ...
    for (const c of res.data.result) {
      ...
      else if (c.phase === "CODING" || c.phase === "PENDING_SYSTEM_TEST")
        status = "active";
      else status = "past";

      // Only include upcoming + recent past (within 30 days)
      const daysDiff =
        (now.getTime() - endTime.getTime()) / (1000 * 60 * 60 * 24);
    ...
    console.log(
      `[Contests] Fetched ${contests.length} Codeforces contests (${contests.filter((c) => c.status === "upcoming").length} upcoming)`,
    );
    return contests;
  } catch (error) {
    console.error(
...
async function fetchCodeChefContests() {
  ...
  try {
    ...
    console.log(
      `[Contests] Fetched ${contests.length} CodeChef contests via Kontests /all`,
    );
    return contests;
  } catch (err) {
    console.warn(
      "[Contests] Kontests /all failed, trying clist.by fallback:",
      err.message,
    );
  }
  ...
  try {
    ...
    console.log(
      `[Contests] Fetched ${contests.length} CodeChef contests via clist.by`,
    );
    return contests;
  } catch (error) {
    console.error(
      "Error fetching CodeChef contests (all sources failed):",
      error.message,
    );
    return [];
  }
...
export const getContests = async (_req, res) => {
  try {
    ...
    const past = allContests
      ...
      .slice(0, 6);
    console.log(
      `[Contests] Returning ${upcoming.length} upcoming, ${active.length} active, ${past.length} past`,
    );
...
export const getContestLeaderboard = async (req, res) => {
  try {
    ...
    const sortedLeaderboard = contest.leaderboard.sort(
      (a, b) => a.rank - b.rank,
    );
    res.json({
      contest: {
