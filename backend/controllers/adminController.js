...
export const getAllUsers = async (_req, res) => {
  try {
    const users = await User.find()
      .select(
        "name email college role xp level profileImage platformIds createdAt",
      )
      .sort({ createdAt: -1 });
    res.json({ users });
  } catch (error) {
...
export const postSpecialQuestion = async (req, res) => {
  try {
    ...
    if (!title || !platform || !problemLink) {
      return res
        .status(400)
        .json({ message: "Title, platform, and problemLink are required" });
    }
...
export const getSpecialQuestions = async (_req, res) => {
  try {
    const questions = await SpecialQuestion.find().sort({ date: -1 }).limit(20);
    res.json({ questions });
  } catch (error) {
    console.error("Get special questions error:", error);
    res
      .status(500)
      .json({ message: "Server error fetching special questions" });
  }
};
...
export const addResource = async (req, res) => {
  try {
    const { title, category, youtubeVideoId, gfgLink, difficulty, order } =
      req.body;
    if (!title || !category) {
      return res
        .status(400)
