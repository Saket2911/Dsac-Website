    ...
    <div className="py-10 animate-in fade-in duration-500 max-w-5xl mx-auto">
      ...
      <div className="relative mb-16">
        ...
        <div className="absolute -bottom-12 left-8 md:left-12 flex items-end gap-6">
          <div className="relative group">
            <Avatar className="h-28 w-28 md:h-32 md:w-32 border-4 border-background bg-background shadow-xl">
              {profileImageUrl && (
                <AvatarImage src={profileImageUrl} alt={displayUser.name} />
              )}
          ...
          <div className="mb-2 hidden md:block pb-12">
            ...
            <p className="text-muted-foreground font-medium flex items-center gap-2">
              {displayUser.college || "No college set"}{" "}
        ...
        {!isPublicView && (
          <div className="absolute bottom-4 right-8 flex gap-3">
            <Button
              variant="outline"
              className="bg-background/80 backdrop-blur text-foreground border-border/50 gap-2"
              onClick={logout}
            >
      ...
      <div className="mb-8 px-4 md:hidden text-center mt-16">
        ...
        <p className="text-muted-foreground text-sm">
          {displayUser.college || "No college set"} &bull; {displayUser.email}
        </p>
      </div>
              ...
              {!isPublicView && loadingStats ? (
                ...
                <>
                  {displayStats.leetcode && (
                    <div className="p-3 rounded-lg hover:bg-primary/5 border border-border/30">
                      <div className="font-semibold text-sm text-foreground mb-1">
                        LeetCode — {displayUser.platformIds?.leetcodeId}
                      </div>
                      <div className="text-xs text-muted-foreground space-y-0.5">
                        <p>
                          Solved: {displayStats.leetcode.totalSolved} (E:
                          {displayStats.leetcode.easySolved} M:
                          {displayStats.leetcode.mediumSolved} H:
                          {displayStats.leetcode.hardSolved})
                        </p>
                        {displayStats.leetcode.contestRating > 0 && (
                          <p>
                            Contest Rating:{" "}
                            {displayStats.leetcode.contestRating}
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  {displayStats.codeforces && (
                    <div className="p-3 rounded-lg hover:bg-primary/5 border border-border/30">
                      <div className="font-semibold text-sm text-foreground mb-1">
                        Codeforces — {displayUser.platformIds?.codeforcesId}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        <p>
                          Rating: {displayStats.codeforces.rating} | Solved:{" "}
                          {displayStats.codeforces.problemsSolved}
                        </p>
                      </div>
                    </div>
                  )}
                  {displayStats.codechef && (
                    <div className="p-3 rounded-lg hover:bg-primary/5 border border-border/30">
                      <div className="font-semibold text-sm text-foreground mb-1">
                        CodeChef — {displayUser.platformIds?.codechefId}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        <p>
                          Rating: {displayStats.codechef.rating} (
                          {displayStats.codechef.stars}) | Solved:{" "}
                          {displayStats.codechef.problemsSolved}
                        </p>
                      </div>
                    </div>
                  )}
                  {displayStats.hackerrank && (
                    <div className="p-3 rounded-lg hover:bg-primary/5 border border-border/30">
                      <div className="font-semibold text-sm text-foreground mb-1">
                        HackerRank — {displayUser.platformIds?.hackerrankId}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        <p>
                          Solved: {displayStats.hackerrank.problemsSolved} |
                          Badges: {displayStats.hackerrank.badges?.length || 0}
                        </p>
                      </div>
                    </div>
                  )}
                  {!displayStats.leetcode &&
                    ...
                    !displayStats.hackerrank && (
                      <p className="text-sm text-muted-foreground text-center py-2">
                        No platform IDs set. Add them in Edit Profile to see
                        your stats.
                      </p>
                    )}
                </>
                  ...
                  {[
                    {
                      ...
                      unlocked: displayUser.xp >= 10,
                    },
                    {
                      name: "Rising Star",
                      icon: "⭐",
                      desc: "Reached Level 5",
                      unlocked: displayUser.level >= 5,
                    },
                    {
                      name: "Dedicated",
                      icon: "🔥",
                      desc: "Earned 500 XP",
                      unlocked: displayUser.xp >= 500,
                    },
                    {
                      name: "Veteran",
                      icon: "🏆",
                      desc: "Reached Level 10",
                      unlocked: displayUser.level >= 10,
                    },
                    {
                      name: "Expert",
                      icon: "💎",
                      desc: "Earned 1000 XP",
                      unlocked: displayUser.xp >= 1000,
                    },
                  ].map((badge, i) => (
                    <div
                      key={i}
                      className={`bg-card border rounded-2xl p-4 text-center transition-shadow group ${badge.unlocked ? "border-primary/30 hover:shadow-md" : "border-border/30 border-dashed opacity-50"}`}
                    >
                      <div
                        className={`w-16 h-16 rounded-full mx-auto flex items-center justify-center text-3xl mb-3 border group-hover:scale-110 transition-transform ${badge.unlocked ? "bg-gradient-to-br from-primary/10 to-primary/30 border-primary/20" : "bg-muted border-muted"}`}
                      >
              ...
              <TabsContent value="edit" className="pt-6">
                <Card className="border-border/50 shadow-sm">
                  ...
                  <CardContent className="space-y-6">
                    ...
                    {saveMessage && (
                      <div
                        className={`text-sm rounded-lg p-3 flex items-center gap-2 ${saveMessage.includes("success") || saveMessage.includes("updated") ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-500 border border-red-200"}`}
                      >
                    ...
                    <div className="flex justify-end gap-4 pt-6">
                      ...
                      <Button
                        className="bg-primary hover:bg-secondary text-white gap-2"
