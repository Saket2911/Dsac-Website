...
type TabType = "xp" | "daily" | "contest" | "platform" | "live-contests" | "daily-tracker";

export default function Leaderboard() {
  const { user, token } = useAuth();
  const [leaderboardData, setLeaderboardData] = useState<LeaderboardEntry[]>([]);
  const [platformData, setPlatformData] = useState<PlatformEntry[]>([]);
  const [liveContests, setLiveContests] = useState<LiveContest[]>([]);
  const [dailyTracker, setDailyTracker] = useState<DailyTrackerData | null>(null);
  ...
  const fetchData = async (type: TabType) => {
    ...
    try {
      ...
      if (type === "platform") {
        const res = await fetch("/api/leaderboard/platform", { headers });
        const data = await res.json();
        setPlatformData(data.leaderboard || []);
      } else if (type === "live-contests") {
        const res = await fetch("/api/leaderboard/live-contests", { headers });
        const data = await res.json();
        setLiveContests(data.contests || []);
      } else if (type === "daily-tracker") {
        const res = await fetch("/api/leaderboard/daily-tracker", { headers });
        const data = await res.json();
        setDailyTracker(data);
      } else {
        const res = await fetch(`/api/leaderboard/${type}`, { headers });
        const data = await res.json();
  ...
  const platformColor = (platform: string) => {
    switch (platform) {
      ...
      case "codechef": return "text-red-500";
  ...
  const platformBg = (platform: string) => {
    switch (platform) {
      case "leetcode": return "bg-amber-500/10 border-amber-500/30";
      case "codeforces": return "bg-blue-500/10 border-blue-500/30";
      case "codechef": return "bg-red-500/10 border-red-500/30";
  ...
  const tabs: { key: TabType; label: string }[] = [
    { key: "xp", label: "By XP" },
    { key: "daily", label: "Daily Solves" },
    { key: "contest", label: "Contests" },
    { key: "platform", label: "Platform Solved" },
    { key: "live-contests", label: "Live Contests" },
    { key: "daily-tracker", label: "Daily Tracker" },
  ];
          ...
          <Card className="border-border/50 shadow-sm max-w-4xl mx-auto overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {(leaderboardData.length < 3 ? leaderboardData : rest).map((entry) => (
                    ...
                    <span className={`w-8 h-8 rounded-full flex items-center justify-center font-serif ${entry.rank === 1 ? "bg-yellow-100 text-yellow-700 font-bold" :
                      ...
                      {entry.rank <= 3 ? (
                        entry.rank === 1 ? <Crown className="w-4 h-4 text-yellow-500" /> :
                          <Medal className={`w-4 h-4 ${entry.rank === 2 ? "text-slate-400" : "text-amber-600"}`} />
                      ) : `#${entry.rank}`}
                    </span>
                  </td>
    ...
    return (
      <div className="space-y-6 max-w-4xl mx-auto">
        {liveContests.map((contest) => (
          <Card key={contest.id} className={`border shadow-sm overflow-hidden ${platformBg(contest.platform)}`}>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {solvers.map((solver, idx) => (
                  <tr key={idx} className="bg-card border-b border-border/40 hover:bg-muted/30 transition-colors">
