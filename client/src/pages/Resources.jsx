...
export default function Resources() {
        ...
        const filtered = searchQuery
          ? items.filter(
              (r) =>
                ...
                subKey.toLowerCase().includes(searchQuery.toLowerCase()),
            )
          : items;
        if (filtered.length > 0) filteredSubs[subKey] = filtered;
      }
      if (Object.keys(filteredSubs).length > 0)
        filteredNested[topKey] = filteredSubs;
    }
      ...
      <div className="py-10 space-y-10 animate-in fade-in duration-500">
        ...
        <div className="bg-card border border-border/50 rounded-3xl p-8 md:p-12 text-center space-y-6 relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-3xl mx-auto space-y-6">
            ...
            <p className="text-muted-foreground text-lg">
              Structured resources organized by topic. Expand each section to
              access YouTube tutorials and practice links.
            </p>
        ...
        <div className="space-y-4 max-w-4xl mx-auto">
          ...
          {Object.entries(filteredNested).map(([topKey, subs]) => {
            ...
            const totalItems = Object.values(subs).reduce(
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {Object.keys(subs).length} topics • {totalItems}{" "}
                        resources
                      </p>
                    </div>
                  </div>
                            ...
                            <div className="divide-y divide-border/20 bg-muted/5">
                              {items.map((resource, idx) => (
                                <div
                                  ...
                                  className="p-4 pl-20 flex items-center gap-4 hover:bg-muted/20 transition-colors group"
                                >
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex gap-2 mt-1">
                                      <Badge
                                        variant="outline"
                                        className={`text-[10px] ${resource.difficulty === "Easy" ? "text-green-500 border-green-200" : resource.difficulty === "Medium" ? "text-amber-500 border-amber-200" : "text-red-500 border-red-200"}`}
                                      >
                                        {resource.difficulty}
                                      </Badge>
                                    </div>
  ...
  const allCategories = [
    "All",
    ...new Set(staticResources.map((r) => r.category)),
  ];
    ...
    <div className="py-10 space-y-10 animate-in fade-in duration-500">
      <div className="bg-card border border-border/50 rounded-3xl p-8 md:p-12 text-center space-y-6 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto space-y-6">
          ...
          <p className="text-muted-foreground text-lg">
            Curated materials, roadmaps, and recorded sessions to support your
            learning journey.
          </p>
          <div className="relative max-w-xl mx-auto mt-8">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
