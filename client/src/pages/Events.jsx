  ...
  upcoming: [
    {
      ...
      attendees: 85,
      description:
        "Learn the fundamentals of designing scalable, distributed systems from industry experts.",
      isRegistered: false,
    },
    {
      id: 2,
      ...
      attendees: 42,
      description:
        "Brainstorming and team formation for the upcoming state-level hackathon.",
      isRegistered: true,
    },
    {
      id: 3,
      ...
      attendees: 156,
      description:
        "Q&A session with recent graduates who secured internships at top tech companies.",
      isRegistered: false,
    },
  ],
  past: [],
...
function EventCard({ event, isFlagship }) {
  return (
    <Card
      className={`border-border/50 shadow-sm transition-all duration-300 ${isFlagship ? "hover:shadow-lg hover:border-primary/30" : "hover:shadow-md"}`}
    >
      <CardContent className="p-0 flex flex-col md:flex-row">
        {/* Date Column */}
        <div
          className={`md:border-r border-b md:border-b-0 border-border/50 p-6 flex md:flex-col items-center justify-center gap-2 md:gap-0 md:min-w-[140px] shrink-0 ${isFlagship ? "bg-gradient-to-b from-amber-50 to-card/50 dark:from-amber-950/20 dark:to-card/50" : "bg-card/50"}`}
        >
          <span
            className={`text-sm font-bold uppercase ${isFlagship ? "text-amber-600 dark:text-amber-400" : "text-primary"}`}
          >
        ...
        <div className="p-6 flex-1 flex flex-col">
          <div className="flex justify-between items-start mb-2">
            {isFlagship ? (
              <Badge
                className={`${getBadgeStyle(event.type)} mb-2 flex items-center`}
              >
              ...
              <Badge
                variant="secondary"
                className="bg-secondary/10 text-secondary mb-2"
              >
                {event.type}
              </Badge>
            )}
        ...
        {tabs.map((tab) => (
          ...
          >
            ...
            {eventsByCategory[tab.key].length > 0 && (
              <span
                className={`ml-1 text-xs rounded-full px-1.5 py-0.5 ${activeTab === tab.key ? "bg-white/20 text-white" : "bg-muted-foreground/10 text-muted-foreground"}`}
              >
