...
export default function Leaderboard() {
  ...
  const renderGenericLeaderboard = () => {
    ...
    const getMetric = (entry) =>
      activeTab === "daily"
        ? `${entry.dailySolved ?? 0} solved`
        : activeTab === "contest"
          ? `${entry.contestsCount ?? 0} contests`
          : `${entry.xp} XP`;
    ...
    return (
      <>
          ...
          <div className="flex justify-center items-end h-64 gap-2 md:gap-6 mt-16 mb-8 px-4">
            ...
            <div className="flex flex-col items-center animate-in slide-in-from-bottom-8 duration-700 delay-100">
              ...
              <div className="font-bold text-foreground text-sm md:text-base">
                <Link href={`/profile/${encodeURIComponent(top3[1].name)}`}>
              ...
              <div className="w-24 md:w-32 h-24 bg-gradient-to-t from-slate-200 to-slate-100 rounded-t-lg border-t-4 border-slate-300 flex items-center justify-center shadow-inner">
                <span className="text-4xl font-serif font-bold text-slate-400">
                  2
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center animate-in slide-in-from-bottom-12 duration-700 z-10">
              ...
              <div className="font-bold text-foreground text-base md:text-lg">
                <Link href={`/profile/${encodeURIComponent(top3[0].name)}`}>
                  <a className="hover:text-primary transition-colors cursor-pointer">
                    {top3[0].name}
                  </a>
                </Link>
              ...
              <div className="w-28 md:w-36 h-32 bg-gradient-to-t from-yellow-200 to-yellow-100 rounded-t-lg border-t-4 border-yellow-400 flex items-center justify-center shadow-inner">
                <span className="text-5xl font-serif font-bold text-yellow-600">
                  1
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center animate-in slide-in-from-bottom-6 duration-700 delay-200">
              ...
              <div className="font-bold text-foreground text-sm md:text-base">
                <Link href={`/profile/${encodeURIComponent(top3[2].name)}`}>
                  <a className="hover:text-primary transition-colors cursor-pointer">
                    {top3[2].name}
                  </a>
                </Link>
        ...
        {/* Rankings Table */}
        {(rest.length > 0 || leaderboardData.length < 3) && (
              ...
              <table className="w-full text-sm text-left">
                <thead className="text-xs text-muted-foreground uppercase bg-card/50 border-b border-border">
                  <tr>
                    <th
                      scope="col"
                      className="px-6 py-4 rounded-tl-lg font-semibold"
                    >
                      Rank
                    </th>
                    ...
                    <th
                      scope="col"
                      className="px-6 py-4 font-semibold text-right rounded-tr-lg"
                    >
                      {metricLabel}
                    </th>
                  </tr>
                ...
                <tbody>
                  {(leaderboardData.length < 3 ? leaderboardData : rest).map(
                    (entry) => (
                      <tr
                        key={entry.rank}
                        className="bg-card border-b border-border/40 hover:bg-muted/30 transition-colors"
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            {user && entry.email === user.email && (
                              <Badge
                                variant="outline"
                                className="ml-2 bg-primary/10 text-primary border-primary/20 text-[10px]"
                              >
                                You
                              </Badge>
                            )}
    ...
    if (platformData.length === 0) {
      return (
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground mt-2">
            Link your platform accounts to appear here!
          </p>
        </div>
      );
    }
    return (
      <Card className="border-border/50 shadow-sm max-w-5xl mx-auto overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-muted-foreground uppercase bg-card/50 border-b border-border">
              <tr>
              ...
              {platformData.map((entry) => (
                ...
                >
                  <td className="px-4 py-4 font-medium">
                    <span
                      className={`w-8 h-8 rounded-full flex items-center justify-center font-serif ${entry.rank === 1 ? "bg-yellow-100 text-yellow-700 font-bold" : entry.rank === 2 ? "bg-slate-100 text-slate-600 font-bold" : entry.rank === 3 ? "bg-amber-100 text-amber-700 font-bold" : "bg-background text-muted-foreground"}`}
                    >
                      {entry.rank <= 3 ? (
                        entry.rank === 1 ? (
                          ...
                          <Medal
                            className={`w-4 h-4 ${entry.rank === 2 ? "text-slate-400" : "text-amber-600"}`}
                          />
                        )
                      ) : (
                        `#${entry.rank}`
                  ...
                  <td className="px-4 py-4 text-center">
                    <span
                      className={`font-bold ${entry.leetcode > 0 ? "text-amber-500" : "text-muted-foreground/40"}`}
                    >
                      {entry.leetcode}
                    </span>
                  </td>
                  <td className="px-4 py-4 text-center">
                    <span
                      className={`font-bold ${entry.codeforces > 0 ? "text-blue-500" : "text-muted-foreground/40"}`}
                    >
                      {entry.codeforces}
                    </span>
                  </td>
                  <td className="px-4 py-4 text-center">
                    <span
                      className={`font-bold ${entry.codechef > 0 ? "text-red-500" : "text-muted-foreground/40"}`}
                    >
                      {entry.codechef}
                    </span>
                  </td>
    ...
    if (liveContests.length === 0) {
      return (
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground mt-2">
            Check back when a contest is running! Rankings are fetched live
            during contests.
          </p>
        </div>
      );
    ...
    return (
      <div className="space-y-6 max-w-4xl mx-auto">
        {liveContests.map((contest) => (
          <Card
            key={contest.id}
            className={`border shadow-sm overflow-hidden ${platformBg(contest.platform)}`}
          >
              ...
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1 text-sm text-muted-foreground">
                    <Badge
                      variant="outline"
                      className={`capitalize ${platformColor(contest.platform)}`}
                    >
                      {contest.platform}
                    </Badge>
                    <span>
                ...
                <Badge
                  className={`text-xs font-bold uppercase ${contest.status === "active" ? "bg-green-500 text-white animate-pulse" : contest.status === "upcoming" ? "bg-blue-500 text-white" : "bg-muted text-muted-foreground"}`}
                >
            ...
            {contest.leaderboard.length > 0 ? (
                    ...
                    {contest.leaderboard.map((entry, idx) => (
                      ...
                      >
                        <td className="px-5 py-3 font-medium">
                          <span
                            className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold ${entry.rank === 1 ? "bg-yellow-100 text-yellow-700" : entry.rank === 2 ? "bg-slate-100 text-slate-600" : entry.rank === 3 ? "bg-amber-100 text-amber-700" : "bg-background text-muted-foreground"}`}
                          >
                            {entry.rank}
                          </span>
                        </td>
              ...
              <div className="p-6 text-center text-muted-foreground text-sm">
                No member rankings available yet. Rankings update every 2
                minutes during active contests.
              </div>
            )}
          </Card>
        ))}
    ...
    if (!dailyTracker || !dailyTracker.question) {
      return (
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground mt-2">
            The daily question hasn't been posted yet. Check back soon!
          </p>
        </div>
      );
    }
    const { question, solvers } = dailyTracker;
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div>
                ...
                <div className="flex items-center gap-3 mt-2">
                  <Badge
                    variant="outline"
                    className={`capitalize ${platformColor(question.platform)}`}
                  >
                    {question.platform}
                  </Badge>
                  <Badge
                    variant="outline"
                    className={difficultyColor(question.difficulty)}
                  >
                    {question.difficulty}
                  </Badge>
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="font-semibold text-foreground">
                          <Link
                            href={`/profile/${encodeURIComponent(solver.name)}`}
                          >
                            <a className="hover:text-primary transition-colors cursor-pointer">
                              {solver.name}
                            </a>
    ...
    <div className="py-10 space-y-12 animate-in fade-in duration-500">
      <div className="text-center max-w-2xl mx-auto space-y-4">
        ...
        <p className="text-muted-foreground text-lg">
          Compete with your peers. Top 3 students at the end of the semester
        ...
        <div className="flex flex-wrap justify-center gap-2 mt-4">
          {tabs.map((tab) => (
            <Button
              ...
              className={`rounded-full text-xs sm:text-sm ${activeTab === tab.key ? "bg-primary/10 text-primary border-primary" : ""}`}
