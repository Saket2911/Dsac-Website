...
export default function DailyQuestion() {
  ...
  const [solvingSpecial, setSolvingSpecial] = useState(null);
  const [specialError, setSpecialError] = useState("");

  // Fetch today's question
  useEffect(() => {
    fetch(`${API_BASE}/daily-question`)
      .then((res) => res.json())
      .then((data) => {
        if (data.question) {
      ...
      .catch(() => setError("Failed to load daily question"))
  ...
  return (
    <div className="py-10 animate-in fade-in duration-500 max-w-5xl mx-auto">
      ...
      {(solvedState === "solved" || solvedState === "already_solved") && (
        <div className="mb-8 p-6 bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200 rounded-2xl flex items-center gap-4 animate-in slide-in-from-top-4 duration-500">
          ...
          <div className="flex-1">
            <h3 className="font-bold text-emerald-900 text-lg flex items-center gap-2">
              {solvedState === "solved" ? (
                <>
                  <Sparkles className="w-5 h-5 text-emerald-500" /> Problem
                  Solved!
                </>
              ) : (
                <>
                  <CheckCircle className="w-5 h-5 text-emerald-500" /> Already
                  Completed
                </>
              )}
            </h3>
            <p className="text-emerald-700 text-sm mt-0.5">
              {solvedState === "solved"
                ? `You earned +${xpAwarded} XP for solving today's daily question!`
                : "You've already solved today's question. Come back tomorrow for a new challenge!"}
            </p>
          </div>
        </div>
      )}

      {/* Special Questions - PROMINENT */}
      {specialQuestions.length > 0 && (
        <div className="mb-10">
          <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-2xl p-6">
            <h2 className="text-xl font-bold text-foreground mb-1 flex items-center gap-2">
              <Star className="w-5 h-5 text-amber-500 fill-amber-500" /> Special
              Challenge
            </h2>
            <p className="text-sm text-muted-foreground mb-4">
              Solve these for bonus XP! Your solution is verified on the
              platform before XP is awarded.
            </p>

            {specialError && (
              <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-600 flex items-center gap-2">
                <AlertCircle className="w-4 h-4 shrink-0" /> {specialError}
              </div>
            )}

            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {specialQuestions.map((sq) => {
                const isSolved = sq.solvedUsers?.some(
                  (s) => s.userId === user?._id,
                );
                return (
                  <div
                    key={sq._id}
                    className="bg-white/70 backdrop-blur rounded-xl border border-amber-100 p-4 flex items-center justify-between gap-4"
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        {sq.solvedUsers?.length || 0} solved
                      </p>
                    </div>
                    <div className="flex flex-col gap-2 shrink-0">
                      <Button
                        ...
                        onClick={() => window.open(sq.problemLink, "_blank")}
                      >
                        <ExternalLink className="w-3 h-3" /> Solve
                      </Button>
                      {isSolved ? (
                        <Badge className="bg-emerald-100 text-emerald-700 border-none text-xs justify-center">
                          <CheckCircle className="w-3 h-3 mr-1" /> Verified
                        </Badge>
                      ) : (
                        <Button
                          size="sm"
                          className="bg-primary hover:bg-secondary text-white text-xs gap-1"
                          disabled={solvingSpecial === sq._id || !token}
                          onClick={async () => {
                            setSolvingSpecial(sq._id);
                            setSpecialError("");
                            try {
                              const res = await fetch(
                                `${API_BASE}/user/solve-special`,
                                {
                                  method: "POST",
                                  headers: {
                                  ...
                                  body: JSON.stringify({ questionId: sq._id }),
                                },
                              );
                              const data = await res.json();
                              if (res.ok) {
                              ...
                              setSolvingSpecial(null);
                            }
                          }}
                        >
                          {solvingSpecial === sq._id ? (
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Solve this to earn XP and climb the leaderboard.
          </p>
        </div>
          ...
          <Card className="border-border/50 shadow-sm">
            <CardHeader className="pb-4">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Badge
                  className={`${diffBadgeClass(question.difficulty)} border-none shadow-none`}
                >
                  {question.difficulty}
                </Badge>
              </div>
            ...
            <CardContent className="space-y-6 text-foreground/90">
              ...
              <div className="border-t border-border/50 pt-5 flex flex-col sm:flex-row justify-between items-center gap-4">
                {solvedState === "solved" ||
                solvedState === "already_solved" ? (
                  <p className="text-sm text-emerald-600 font-medium flex items-center gap-2">
                    <CheckCircle className="w-4 h-4" /> Completed — Great work!
                  </p>
                ) : (
                  <p className="text-sm text-muted-foreground">
                    Solve this problem on the platform. Come back and click
                    "Check Solution" to earn XP.
                  </p>
                )}
                <div className="flex gap-3 shrink-0">
                  {solvedState !== "solved" &&
                    solvedState !== "already_solved" && (
                      <Button
                        ...
                        disabled={solvedState === "checking" || !token}
                      >
                        {solvedState === "checking" ? (
                  ...
                  <Button
                    className="bg-primary hover:bg-secondary text-primary-foreground gap-2 rounded-full px-6"
                    onClick={() => window.open(question.platformUrl, "_blank")}
                  >
                    <ExternalLink className="w-4 h-4" /> Solve on{" "}
          ...
          <Card className="border-border/50 shadow-sm">
            ...
            <CardContent className="pt-4 space-y-3">
              <div className="flex justify-between items-center">
                ...
                <Badge
                  className={`${diffBadgeClass(question.difficulty)} border-none shadow-none text-xs`}
                >
                  {question.difficulty}
                </Badge>
              </div>
              ...
              <CardContent className="pt-4 space-y-3">
                {question.hints.map((hint, i) => (
                  <details key={i} className="group">
                    ...
                    <div
                      className="mt-2 px-3 pb-2 text-xs text-muted-foreground leading-relaxed"
                      dangerouslySetInnerHTML={{
                        __html: hint,
                      }}
                    />
            ...
            <CardContent className="pt-5 pb-5">
              <div className="flex gap-3">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">
                  {!token ? (
                    <span>
                      Sign in to track your progress and earn XP for solving
                      this challenge.
                    </span>
                    ...
                    <span>
                      ...
                      in your Profile to auto-verify solutions and earn XP.
                    </span>
                  ) : (
                    <span>
                      Solve the problem on LeetCode, then click{" "}
                      <span className="font-semibold text-foreground">
                        "Check Solution"
                      </span>{" "}
                      to verify and earn{" "}
