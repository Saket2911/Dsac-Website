...
export default function Contests() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API_BASE}/contests`).then((r) => r.json()),
      fetch(`${API_BASE}/contests/member-count`).then((r) => r.json()),
    ])
      .then(([contestData, memberData]) => {
    ...
    return (
      <div className="py-20 flex flex-col items-center justify-center gap-4 animate-in fade-in">
        ...
        <p className="text-muted-foreground">
          Fetching contests from platforms...
        </p>
      </div>
    );
  }
  const nextContest =
    active.length > 0 ? active[0] : upcoming.length > 0 ? upcoming[0] : null;
  return (
    <div className="py-10 space-y-12 animate-in fade-in duration-500">
          ...
          <CardContent className="p-8 md:p-12 relative z-10 flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="flex-1 text-center md:text-left space-y-4">
              <div className="flex flex-wrap justify-center md:justify-start gap-2">
                <Badge
                  className={`${active.length > 0 ? "bg-red-100 text-red-700 animate-pulse" : "bg-green-100 text-green-700"} hover:bg-current border-none px-3 py-1 font-semibold uppercase tracking-wider text-xs`}
                >
                  {active.length > 0 ? "Live Now" : "Upcoming"}
                </Badge>
                <Badge
                  className={`${platformColors[nextContest.platform]?.bg || "bg-gray-100"} ${platformColors[nextContest.platform]?.text || "text-gray-700"} border-none text-xs`}
                >
                  {platformColors[nextContest.platform]?.label ||
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-4 text-sm font-medium pt-2">
                ...
                <div className="flex items-center gap-1.5 text-foreground/80 bg-background/50 px-3 py-1.5 rounded-md border border-border/50">
                  ...
                  {formatTime(nextContest.startTime)}
                </div>
                <div className="flex items-center gap-1.5 text-foreground/80 bg-background/50 px-3 py-1.5 rounded-md border border-border/50">
                  <Clock className="w-4 h-4 text-primary" />{" "}
                  {formatDuration(nextContest.duration)}
                </div>
              </div>
              <div className="pt-4">
                <Button
                  size="lg"
                  className="bg-primary hover:bg-secondary text-white rounded-full px-8 shadow-md gap-2"
      ...
      {upcoming.length > 1 && (
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {upcoming.slice(1, 7).map((contest, idx) => (
              <Card
                key={idx}
                className="border-border/50 shadow-sm hover:shadow-md transition-all duration-300 group"
              >
                <CardHeader className="pb-3 border-b border-border/30 bg-card/30">
                  <div className="flex justify-between items-start">
                    ...
                    <Badge
                      className={`${platformColors[contest.platform]?.bg || "bg-gray-100"} ${platformColors[contest.platform]?.text || "text-gray-700"} border-none text-xs shrink-0 ml-2`}
                    >
                      {platformColors[contest.platform]?.label ||
                        contest.platform}
                    </Badge>
                  </div>
                </CardHeader>
      ...
      {past.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {past.slice(0, 6).map((contest, idx) => (
              <Card
                key={idx}
                className="border-border/50 shadow-sm hover:shadow-md transition-all duration-300 group opacity-80 hover:opacity-100"
              >
                <CardHeader className="pb-3 border-b border-border/30 bg-card/30">
                  <div className="flex justify-between items-start">
                    <div className="flex-1 min-w-0">
      ...
      {upcoming.length === 0 && active.length === 0 && past.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground mt-2">
            Check back soon for upcoming contests from LeetCode and Codeforces.
          </p>
        </div>
      )}
    </div>
