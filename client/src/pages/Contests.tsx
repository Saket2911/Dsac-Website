...
const platformColors: Record<string, { bg: string; text: string; label: string }> = {
  codeforces: { bg: "bg-blue-100", text: "text-blue-700", label: "Codeforces" },
  leetcode: { bg: "bg-amber-100", text: "text-amber-700", label: "LeetCode" },
  codechef: { bg: "bg-orange-100", text: "text-orange-700", label: "CodeChef" },
};
...
export default function Contests() {
  const [upcoming, setUpcoming] = useState<ContestData[]>([]);
  const [active, setActive] = useState<ContestData[]>([]);
  const [past, setPast] = useState<ContestData[]>([]);
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/contests/member-count").then((r) => r.json()),
    ])
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {upcoming.slice(1, 7).map((contest, idx) => (
              <Card key={idx} className="border-border/50 shadow-sm hover:shadow-md transition-all duration-300 group">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {past.slice(0, 6).map((contest, idx) => (
              <Card key={idx} className="border-border/50 shadow-sm hover:shadow-md transition-all duration-300 group opacity-80 hover:opacity-100">
                <CardHeader className="pb-3 border-b border-border/30 bg-card/30">
                  <div className="flex justify-between items-start">
                    <div className="flex-1 min-w-0">
                      <CardTitle className="text-lg truncate">{contest.name}</CardTitle>
