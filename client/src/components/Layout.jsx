            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="rounded-full text-foreground/70 hover:text-primary hover:bg-primary/10 p-0 overflow-hidden"
                >
                  {profileImageUrl ? (
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden border-t border-border bg-background py-4 px-4 shadow-lg absolute w-full left-0 max-h-[80vh] overflow-y-auto">
            <nav className="flex flex-col gap-2">
              ...
              <Link href="/profile">
                <Button
                  className="bg-primary w-full text-primary-foreground mt-2"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  Profile
                </Button>
              ...
              {isAuthenticated && (
                <Button
                  variant="ghost"
                  className="w-full text-destructive hover:bg-destructive/10 mt-1 gap-2"
                  onClick={() => {
                    logout();
        ...
        <div className="container mx-auto max-w-7xl px-4 py-12 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-muted-foreground font-medium">
              &copy; {new Date().getFullYear()} DSaC. Empowering students
              through technology.
            </p>
          </div>
